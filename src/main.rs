...
#[derive(Clone, Copy, PartialEq)] enum Orient { Vertical, Horizontal }
impl Orient {
    fn from_env() -> Self {
...
fn main() -> Result<()> {
    ...
    let eq_tau: f32 = env::var("LOOKAS_EQ_TAU").ok().and_then(|s| s.parse().ok()).unwrap_or(3.0);          // seconds
    let eq_strength: f32 = env::var("LOOKAS_EQ_STRENGTH").ok().and_then(|s| s.parse().ok()).unwrap_or(0.80); // 0..1
